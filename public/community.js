...
function closeSwal() { document.getElementById("swal").classList.remove("show"); }
function save() { localStorage.setItem("ef_posts", JSON.stringify(posts)); }

function addPost() {
    if (!title.value.trim() || !content.value.trim()) {
        showSwal("⚠ Please fill all fields"); return;
    }
    posts.unshift({ id: Date.now(), title: title.value, content: content.value, up: 0, replies: [] });
    title.value = ""; content.value = "";
    save(); render(); showSwal("✅ Post added");
}

function delPost(id) {
    posts = posts.filter(p => p.id !== id); save(); render(); showSwal("🗑 Post deleted");
}

function upvote(id) { posts.find(p => p.id === id).up++; save(); render(); }

function reply(id) {
    const i = document.getElementById("r" + id);
    if (!i.value.trim()) { showSwal("⚠ Reply empty"); return; }
    posts.find(p => p.id === id).replies.push({ t: i.value, u: 0 });
    i.value = ""; save(); render(); showSwal("💬 Reply added");
}
